#!/usr/bin/env python3
"""Strict independent SciPy pipeline used by the reproducibility audit.

The pipeline receives only ``RawSpectralProblem``.  It does not receive a native
window, mesh, bracket, operator, witness, or reference value.  Unlike the first
competition pipeline, the well search is not restricted to a fixed [-32, 32]
box: it expands finite probes until the sampled minimum is safely interior, then
uses SciPy bounded minimization only inside the discovered cell.
"""
...
from retained_spectral.engine import RawSpectralProblem, RawSpectralResult, potential_values
...
def _scipy_tridiagonal(
    ...
    off_diagonal = np.full(intervals - 2, -0.5 / spacing**2, dtype=np.float64)
    return diagonal, off_diagonal


def _scalar_potential(problem: RawSpectralProblem, value: float) -> float:
    return float(potential_values(problem, np.asarray((value,), dtype=np.float64))[0])


def _discover_well(problem: RawSpectralProblem) -> tuple[float, float]:
    """Independent expanding probe search followed by a local SciPy minimizer."""
...
def _initial_window(problem: RawSpectralProblem) -> tuple[float, tuple[float, float]]:
    ...
    while intervals <= max_intervals:
        ...
        if len(spectra) >= 3:
            ...
            if float(np.max(shift)) <= 0.45 * problem.tolerance:
                return richardson_fine, shift, solve_intervals[-1], solves, peak_bytes
        intervals *= 2
    if len(spectra) < 3:
...
def scipy_raw_input_readout_strict(
    problem: RawSpectralProblem,
    *,
    max_intervals: int = 1_048_576,
    max_window_rounds: int = 10,
) -> RawSpectralResult:
    """End-to-end independent SciPy pipeline from the same raw input."""
    ...
    for _round in range(max_window_rounds):
        ...
        expanded, tail_pass = _expanded_window(
            problem, window, float(values[-1]), scale
        )
        ...
        (
            ...
            solves,
            working,
        ) = _richardson_mesh(problem, expanded, expanded_initial, max_intervals)
        total_solves += solves
        peak_bytes = max(peak_bytes, working)
    ...
    return RawSpectralResult(
        method="Strict independent SciPy Richardson-stability pipeline",
        status=status,
...
__all__ = ["scipy_raw_input_readout_strict"]
