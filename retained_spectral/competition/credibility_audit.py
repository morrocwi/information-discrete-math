#!/usr/bin/env python3
"""Adversarial and scaling audit for the strict Retained Spectral benchmark.
...
from retained_spectral.competition.strict_run import run_strict_competition
...
def run_adversarial() -> dict[str, object]:
    ...
    for target in adversarial_targets():
        ...
        for label, solver in (
            ...
            ("scipy", scipy_raw_input_readout_strict),
        ):
...
def _cold_command(kind: str) -> str:
    solver_import = (
        ...
        if kind == "native"
        else "from retained_spectral.competition.scipy_pipeline_strict import "
        "scipy_raw_input_readout_strict as solve"
    )
    return f"""
import json, time
...
def run_credibility_audit(
    *,
    ...
    baseline = run_strict_competition(
    ...
    gates = {
        "strict_baseline_accept": baseline["verdict"] == "ACCEPT",
    ...
    verdict = "ACCEPT" if all(gates.values()) else "HOLD"
    return {
        "schema": "idm.retained-spectral-credibility-audit.v1",
        "simulation": False,
        "environment": {
        ...
        "verdict": verdict,
        "scope_note": (
            "ACCEPT supports reproducibility and correctness for the declared and "
            "adversarial one-dimensional finite-diagnostic suite. Scaling data is "
            "reported across k and N; it does not imply universal solver dominance."
        ),
    }
