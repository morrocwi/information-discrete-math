"""idm.kernel.cas — the EXACT symbolic engine (our own, not SymPy). THE HOME of the symbolic CAS.

Migrated here from ``idm/symbolic.py`` (P1.7): the algorithms now live in the kernel package — the one
substrate — and ``idm.symbolic`` is a thin compatibility re-export so every existing ``from idm import
symbolic`` / ``SYM.*`` call site keeps working byte-identically. The six ``symbolic_*`` solver kinds
therefore now compute through ``idm.kernel``.
