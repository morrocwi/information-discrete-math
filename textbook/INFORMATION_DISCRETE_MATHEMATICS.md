...
- **Part XXII** — The root design: three axioms that generate the solver (`READOUT · FOLD · DECISION` + the `KEYSTONE` bridge; the 230-kind branch map — 259 live with the Hilbert core and the `polynomial_positivity` certificate solver; reductions machine-checked in `IDM_Reduction.v`)
...
> (`RD.v`, `RDL.v`, …) point to the **external machine-checked corpora** — `solver-arc-private`,
...
## 2.1 The primitive (Def 2.1, `Dr` → `Th_coqc` realization) [Toledo: delta_R]

**`δ_R`** — *a retained difference exists*. Ontologically primitive; nothing is assumed below it.
Its realization is machine-checked: **Th 2.1 (`primordial_difference_exists` [Toledo: delta_R/M.03.v1], `Th_coqc`)** — a first
distinction exists, witnessed by `succ 0 ≠ 0`.

## 2.2 Axioms of Retained Difference (Ax-RD1…RD9) [Toledo: RD1, RD2, RD3, RD4, RD5, RD6, RD7, RD8, RD9]
...
## 2.3 The naturals `D` [Toledo: D] (`Th_coqc`, axiom-free — witness `RD.v`, `RDL_Distinguishability.v` in the
sister repo `solver-arc-private/formal/`, NOT local to this repo's own `formal/`; verified
directly — `coqc -q RD.v` then `Print Assumptions <name>` reports *Closed under the global
context* for every theorem below except `Con_PA_classical`, see Th 2.7)
...
- **Th 2.5 (elementary equivalence).** `D ≡ ℕ` via a full first-order satisfaction transfer
  (witness `sat_transfer`/`sentence_transfer`). **Tier note (see §10.3's downgrade):** the Coq
  proof itself is machine-checked axiom-free, but §10.3 flags the *statement* as `+ℝ-Open` on
  philosophical grounds (it quantifies over a completed model of `D`) rather than on proof-gap
  grounds. Both are stated here rather than silently picking one — reconciling which tier governs
  a Coq-checked but completed-totality-quantified statement is an open call for this repo's
  author, not settled by re-running the proof.
- **Th 2.6 (`D` models PA; `Con_PA`).** `D ⊨` first-order Peano Arithmetic (witness `D_models_PA`),
  and **PA is consistent** `Con_PA : ¬ Prov PA ⊥` (consistency-from-a-model), axiom-free. The
  `Con_PA` half is unconditionally `Th_coqc` by any reading (it is the non-collapse of `D`'s own
  structure, not a claim about a completed model). The `D ⊨ PA` half carries the same tier note as
  Th 2.5 above.
- **Th 2.7 (`Con_PA_classical`, `Th_coqc+classic`).** The classical frontier of consistency is closed
  on the single axiom `classic`. A parameterized variant that removes even this axiom exists —
  `solver-arc-private/formal/RD_ConPA_ReadoutBivalence.v: Con_PA_classical_param` — verified
  directly: `Print Assumptions Con_PA_classical_param` reports *Closed under the global context*
  (no `classic`), via an explicit, disclosed `ReadoutComplete` hypothesis instead of a global axiom.
...
## 3.1 The integers `ℤ` [Toledo: Z] (`Th_coqc`, axiom-free — Grothendieck completion of `D`)
...
## 3.3 The rationals `ℚ` [Toledo: Q] (`Th_coqc`, axiom-free — field of fractions of `ℤ`)
...
## 3.4 The reals `ℝ` [Toledo: R] — the continuum AS a readout (`Th_coqc`, axiom-free unless noted; Toledo tiers this rung `+ℝ-axioms`, not `Th_coqc`, since it imports `Coq.Reals`)
...
## 4.3 The relation graph and the operator `L_R` [Toledo: L_R] (`Th_coqc`, axiom-free — `RDL_CausalOrder.v`)
...
- **`π` = the calibrated invariant of compact retained closure** (order-reversal witness `Th_coqc`-elig; the metric-calibration route is `+ℝ-Open`). Two roots,
  ...
  `formal/IDM_Harvest.v: odd_from_cyclic_closure`/`least_nontrivial_odd_is_three` [Toledo: L_R/M.15.v1] — the same `k`-odd witness that forces
...
## 5.1 The keystone (Def 5.1 + Th 5.1) [Toledo: Keystone]
...
- **Th 5.1 (keystone — `Th_coqc`, witness `formal/IDM_Keystone.v: keystone_B_eq_I`).** **`B(Φ,Φ) = I(Φ)`**:
  ...
  (`Print Assumptions` = *Closed under the global context*); its companion `keystone_nonneg` [Toledo: Keystone/M.07.v1] shows
...
keystone, §5.1 `keystone_nonneg` [Toledo: Keystone/M.07.v1]) makes the homogeneous energy non-increasing (`relaxation_dissipation` [Toledo: Keystone/M.08.v1]).
...
witnesses** (Th 10.1 `kuratowski_pair_inj` [Toledo: delta_R/M.02.v1]; **Th 10.2 `formal/IDM_SetsFunctions.v` — the function ≅
functional-relation coincidence, converted from a sketch in the v-proofs edition**; Th 10.3–10.5
`IDM_FiniteWitnesses2.v`; Th 10.6 propositional `IDM_Logic.v` **and first-order `formal/IDM_FirstOrder.v`
— the finite-domain quantifier recursion, also converted in the v-proofs edition**; Th 10.7
`IDM_Logic.v`), each confirmed `Closed under the global context` by `bash formal/verify.sh`. What remains a *declared* frontier is §10.5's `ε→0` real-analysis
...
is `solver-arc-private/formal/RD.v` (sister repo, not local — `RD3_succ_ne_zero`,
`RD4_succ_inj`, `add_assoc`, `add_comm`, `mul_add` distributive, `Th_coqc`); only the Kuratowski
pair / product / function step below is the new sketch.
...
- **Function** = a relation `R` with `IsFunction(R) :≡ ∀a∈X ∃!_λ b∈Y, (a,b)∈R` (`∃!_λ` a finite
  ...
  Machine-checked: the graph of a map on a finite domain is **total** (`graph_total` [Toledo: delta_R/M.07.v1]) and
  **single-valued** (`graph_single_valued` [Toledo: delta_R/M.08.v1]) — i.e. a function; the correspondence is **faithful**, maps
  agree on the domain iff their graphs coincide (`graph_faithful` [Toledo: delta_R/M.09.v1] / `graph_faithful_converse` [Toledo: delta_R/M.10.v1]); and the
  converse construction recovers a map from any functional relation (`functional_relation_has_map` [Toledo: delta_R/M.11.v1]).
...
- **Potential infinite** = a `δ_R`-generated tape `Tape(n+1)=σ(Tape n)`, each stage finite with
  `Counting(Tape n)=n+1`. **Th 10.4 (`Th_coqc`, witness `formal/IDM_FiniteWitnesses2.v: tape_count_succ` / `tape_no_terminal` [Toledo: D/M.44.v1]):**
...
- **Th 10.6 (`Th_coqc` — propositional witness `formal/IDM_Logic.v: finite_satisfaction_dec` / `sat_dec` [Toledo: D/M.57.v1], decidable model-checking over `¬∧∨`; FIRST-ORDER witness `formal/IDM_FirstOrder.v: eval_correct` / `sat_fo_decidable` [Toledo: D/M.53.v1] — the quantifier-over-finite-domain Tarski recursion `∀↦forallb`, `∃↦existsb`, proved sound+complete and decidable, axiom-free):** `⊨_λ` — a decidable, computable Tarski recursion of satisfaction on a
  finite domain + finite formula at a fixed coarse-graining level. **Existing witness to REUSE (economize):**
  the readout-bivalence consistency layer is already machine-checked — sister repo
  `solver-arc-private/formal/RD_ConPA_ReadoutBivalence.v` (`soundnessC_param`,
  `consistencyC_param`, `Con_PA_classical_param`, `Th_coqc` — verified directly, `Print Assumptions`
  reports *Closed under the global context* for both, no `classic`); only the general finite `⊨_λ`
  recursion is the new sketch, and it builds on that file.
- **Downgrade (honest, tier note — see also §2.3 Th 2.5/Th 2.6):** the earlier "`D ⊨ PA`" (full
  first-order Peano) and "`D ≡ ℕ`" (elementary equivalence / nonstandard models) are flagged
  **`+ℝ-Open`** here — they quantify over the completed standard model, and are NOT *reproven from
  scratch in this local §10.3 development*; only `⊨_λ` is a new claim here. This is a philosophical
  downgrade about completed-totality quantification, not a claim that the underlying Coq proofs
  (`sat_transfer`/`sentence_transfer`/`D_models_PA` in `solver-arc-private/formal/RD.v`) are
  unverified — they are machine-checked axiom-free (§2.3). Whether a Coq-checked ∀-statement over
  `D` still counts as smuggling a completed totality under this repo's own readout-first stance is
  an open tier call, stated here rather than resolved silently in either direction.

## 10.4 RDL boundary-tie — making the logic load-bearing

- **Th 10.7 (RDL non-explosion is `Th_coqc`, witness `formal/IDM_Logic.v: rdl_non_explosion` — a Belnap–Dunn 4-valued countermodel where `p∧¬p` is designated but `q` is not, with `classical_would_explode` [Toledo: delta_R/M.05.v1] as the 2-valued control; that `D`'s semiring closure *requires* this is the `Dr` boundary-tie):** `D`'s semiring closure **requires RDL's non-explosion**, not classical
...
- **Th 10.8 (`Th_coqc` AS STATED — the exact finite-`ε` algebra, zero `O(ε)` residue; local witnesses
  `formal/IDM_Calculus.v`):** `D_ε(f+g)=D_εf+D_εg` (`delta_sum` [Toledo: Z/M.01.v1]/`Deps_sum` [Toledo: Z/M.04.v1]) · `D_ε(c·f)=c·D_εf`
  (`delta_scalar` [Toledo: Z/M.02.v1]) · the causal `D_ε(f·g)[n]=f[n+1]·D_εg[n]+g[n]·D_εf[n]` (`delta_product` [Toledo: Z/M.03.v1]/`Deps_product` [Toledo: Z/M.05.v1]) ·
  **FTCC** `I_ε(D_εf)[N]=f[N]−f[0]` exactly (`FTCC_telescope` [Toledo: Z/M.06.v1]) · **summation by parts**
  `Σ(f[k+1]Δg[k]+g[k]Δf[k])=f[N]g[N]−f[0]g[0]` (`summation_by_parts` [Toledo: Z/M.09.v1]). Each discharges by `field`/`ring` /
...
## 10.8 Mined confirmations from the machine-checked corpus (solver-arc-private)
...
- **RD1–RD9 → `D ≅ ℕ`** (Peano-compatibility) · **RDL** 4-valued paraconsistent core (Belnap–Dunn FDE) ·
  ...
  **DEC toolkit** (discrete exterior calculus around `L_R`, 9 tools, minimal-cell scope) · **π and φ as
  readout-invariants** (reconstruction limits, not root objects; `Dr`). *(Witnesses, sister repo
  `solver-arc-private/formal/`, not local to this repo: `RD.v`, `RDL.v`,
...
- **Group axioms are theorems, not decrees.** Composition of readout-preserving relabelings is
  ...
  (`Th_coqc`, witness `formal/IDM_FiniteWitnesses3.v: aut_assoc`/`aut_id_left` [Toledo: D/M.48.v1]/`aut_id_right` [Toledo: D/M.49.v1]/`aut_inv_left` [Toledo: D/M.50.v1]/`aut_inv_right` [Toledo: D/M.51.v1] — bijective readout-preserving relabelings are closed under `∘`, with identity and two-sided inverse):** `(Aut(F,O), ∘)` satisfies the group axioms. No axiom is imported; the group *is* the symmetry of a reading.
...
`formal/IDM_Harvest.v: sym_skew_reconstruct`, `sympart_self_adjoint` [Toledo: L_R/M.17.v1], `skew_diag_zero` [Toledo: L_R/M.19.v1]): `M = ½(M+Mᵀ) +
...
(§15.2 `laplacian_symmetric` [Toledo: L_R/M.20.v1]), so it *is* its own metric part.
...
`formal/IDM_Matrix.v` (`transpose_mmul` [Toledo: Q/M.11.v1], `mid_left` [Toledo: Q/M.12.v1], `laplacian_symmetric` [Toledo: L_R/M.20.v1], `laplacian_rowsum_zero` [Toledo: L_R/M.21.v1],
`laplacian_ones_in_kernel` [Toledo: L_R/M.22.v1]), all axiom-free. Eigenvalue/eigenvector computation for the finite `L_R` is
...
twirl_idempotent`, via `trace_twirl` [Toledo: Q/M.17.v1]) — whose image is the **one-dimensional scalar line** `ℚ·I`
(`twirl_image_scalar` [Toledo: Q/M.13.v1], `scalar_line_one_dim` [Toledo: Q/M.19.v1]). So a full metric on an `n`-dimensional retained space —
...
- **Connectivity = `ker L_R`.** The number of connected components equals `dim ker L_R` (constant-on-
  component vectors) — the kernel is literally the indistinguishability space (§V). The **constants-in-kernel** direction is machine-checked (`Th_coqc`, witness `formal/IDM_Matrix.v: laplacian_ones_in_kernel`, via row-sum zero `laplacian_rowsum_zero` [Toledo: L_R/M.21.v1]); the full rank-nullity count of components over `ℚ` is `Th_coqc`-elig.
...
- **Pigeonhole and Ramsey-type bounds** are finite existence readouts: with more distinctions than
  ...
  `formal/IDM_FiniteWitnesses.v: pigeonhole`; the handshake identity `Σdeg=2|E|` is `handshake_lemma` [Toledo: L_R/M.01.v1]).
...
- **Bounded operator = finite retained coupling.** On the `ℚ`-space, an operator is a finite matrix;
  ...
  semidefinite (§5.1 `keystone_nonneg` [Toledo: Keystone/M.07.v1]). As in §13.3, the finite spectral content that is a *readout* is
...
- **Exact core (`Th_coqc`, witness `formal/IDM_Bridge.v`).** For the fundamental operation — the
  ...
  `FTCC_exact` [Toledo: R/M.09.v1] / `FTCC_eps_exact` [Toledo: R/M.10.v1], proved axiom-free over `ℚ`; the `ε` cancels for every declared
...
- **inner product** `⟨u,v⟩ = Σ conj(uᵢ)vᵢ`, exact; sesquilinear + PSD — machine-checked (`inner_sym` [Toledo: Keystone/P.04.v1],
  `inner_linear_l` [Toledo: Keystone/P.05.v1], `inner_pos` [Toledo: Keystone/P.06.v1]). **Cauchy–Schwarz** (`cauchy_schwarz_2` [Toledo: Keystone/P.09.v1]), **parallelogram law**,
  **Pythagoras** (`pythagoras_orthogonal` [Toledo: Keystone/P.08.v1]) — all axiom-free ℚ.
...
- **projection** `P=A(A*A)⁻¹A*` (exact), idempotent + self-adjoint (`projection_idempotent` [Toledo: Keystone/P.14.v1],
  `projection_self_adjoint` [Toledo: Keystone/P.15.v1]). **adjoint / self-adjoint / unitary / normal** — decidable exact ℚ[i]
  equalities; `(A*)* = A` and `(AB)* = B*A*` machine-checked (`adjoint_involutive` [Toledo: Keystone/P.10.v1], `adjoint_of_product` [Toledo: Keystone/P.11.v1]).
- **characteristic polynomial** — exact ℚ[i] coefficients (Faddeev–LeVerrier), always exact regardless
  ...
  `hermitian_2x2_discriminant_nonneg` [Toledo: Keystone/P.12.v1] (`0 ≤ (a−c)²+4b²`) and `hermitian_2x2_gap_is_discriminant` [Toledo: Keystone/P.13.v1]
...
the shortest set of axioms from which the book's *computational* content, and the branch half of the
solver `idm` (the 230-kind branch map; 258 with the Part XXI.5 Hilbert core, 259 live including the `polynomial_positivity` certificate solver), is produced. The claim of this part is deliberately narrow and tier-honest: **three axioms plus
...
- **A2 · FOLD** [Toledo: A2] (`Th_coqc`, the engine — Part VIII, §22.3).
...
- **A3 · DECISION** [Toledo: A3] (`Th_coqc`, the search — §22.3).
...
- **A4 · KEYSTONE** [Toledo: Keystone] (`Th_coqc`, the bridge — Part V, Th 5.1).
...
|---|---|
| `ftcc_Z` [Toledo: A2/M.03.v1] | analysis / summation branch `= fold` over `(+)`; telescoping `I(Df)=f[N]−f[0]` exact |
| `foldmin_le_init` [Toledo: A2/M.04.v1], `foldmin_le_elem` [Toledo: A2/M.05.v1] | shortest-path / DP branch `=` the **same** fold over `(min)` (relaxation invariant) |
| `foldmax_ge_init` [Toledo: A2/M.12.v1], `foldmax_ge_elem` [Toledo: A2/M.13.v1] | critical- / widest-path branch `=` the same fold over `(max)` |
| `sum_is_fold` [Toledo: A2/M.06.v1], `path_is_fold` [Toledo: A2/M.07.v1] | the two kernels are **definitionally one `fold`**, differing only in `(⊕,⊗)` |
| `fold_linear` [Toledo: A2/M.10.v1], `fold_add_split` [Toledo: A2/M.11.v1] | the semiring laws (linearity, additivity) that *make* the reduction valid |
| `dot_is_fold` [Toledo: A2/M.17.v1], `dot_scale` [Toledo: A2/M.18.v1] | inner product / matrix-multiply / convolution kernel `= fold` of products |
| `pivot_preserves` [Toledo: A2/M.08.v1] | linear-algebra / LP / determinant-sign geometry atom: a retained-difference row op preserves every solution ⇒ elimination is a schedule of folds |
| `horner_scaled` [Toledo: A2/M.19.v1], `horner_is_poly` [Toledo: A2/M.20.v1] | polynomial branch: Horner's method **is** a (right) fold and computes the polynomial `Σ cₖ xᵏ` exactly |
| `foldmin_in` [Toledo: A2/M.21.v1] | shortest-path exactness: the accumulated minimum is **attained** (equals the start or an actual element), not merely a lower bound |
| `fold_add_app` [Toledo: A2/M.22.v1] | the associative-monoid chunking law: a fold splits over concatenation ⇒ associative reductions parallelise (MapReduce), order-independent |
| `factorial_is_fold` [Toledo: A2/M.23.v1] | product / combinatorics branch: `n!` is the same fold over `(×)` |
| `witness_sound` [Toledo: A3/M.01.v1], `witness_complete` [Toledo: A3/M.02.v1], `decide_reflect` [Toledo: A3/M.06.v1], `decide_dec` [Toledo: A3/M.03.v1] | A3: a bounded checkable witness is a decidable ACCEPT/HOLD, faithfully reflected |
| `witness_composite_sound` [Toledo: A3/M.04.v1], `composite_has_factor` [Toledo: A3/M.05.v1] | number-theory kernel: a found divisor **proves** compositeness (the `is_prime` backbone) |
| `witness_power_sound` [Toledo: A3/M.07.v1] | `integer_root` / `is_perfect_square` kernel: a found `w` with `w^k=n` proves a perfect power |
| `witness_qr_sound` [Toledo: A3/M.08.v1], `witness_dlog_sound` [Toledo: A3/M.09.v1] | crypto kernels: a found `w` with `w²≡a` / `g^w≡h (mod p)` solves the modular square root / discrete logarithm |
| `sat_reduces_to_decision` [Toledo: A3/M.10.v1], `sat_model_sound` [Toledo: A3/M.11.v1] | the flagship decision problem: CNF **satisfiability** over the `2ⁿ` assignments is the A3 schema; a returned model provably satisfies every clause |
| `relax_nonincreasing` [Toledo: A2/M.24.v1], `relax_idempotent` [Toledo: A2/M.25.v1] | Bellman–Ford / DP **convergence**: min-plus relaxation is non-increasing and idempotent per edge ⇒ the fixpoint is reached in finite rounds |
| `weak_duality_2` [Toledo: A2/M.26.v1] | **A2 × A3 bridge** — LP weak duality: a dual-feasible `y ≥ 0` is a *certificate* (A3) that bounds the simplex objective (A2), `c·x ≤ y·b` |
| `fold_right_perm` [Toledo: A2/M.02.v1] (+ `op_swap` [Toledo: A2/M.01.v1]) | the **deep semiring law**: a fold over an associative-commutative operation is invariant under any reordering (Permutation) of its inputs — the licence to reorder / chunk / parallelise, so A2 is well-defined independent of traversal order |
| `fold_split_even_odd` [Toledo: A2/M.27.v1] | the **FFT** divide-and-conquer split: the sum over `[0,2n)` decomposes into the even-index and odd-index sub-folds (the recursive decomposition the fast transform is built on) |
| `modpow_is_fold` [Toledo: A2/M.28.v1] | crypto forward map: modular exponentiation `g^N mod p` is the same fold over `(× mod p)` — the RSA / EC / discrete-log engine |
| `foldmax_in` [Toledo: A2/M.14.v1] | critical-/widest-path exactness: the accumulated maximum is **attained** (mirror of `foldmin_in` [Toledo: A2/M.21.v1]) |
| `sum_list_perm` [Toledo: A2/M.15.v1], `prod_list_perm` [Toledo: A2/M.16.v1] | set-aggregation is **order-independent**: `Σ` and `Π` over a multiset are well-defined (corollaries of `fold_right_perm` [Toledo: A2/M.02.v1]) — the basis of `describe`/statistics and combinatorial products |
| `tautology_sound` [Toledo: A3/M.12.v1], `cnf_tautology_sound` [Toledo: A3/M.13.v1] | the **universal** counterpart of the witness search — `truth_table` / validity: a bounded `∀` over the `2ⁿ` rows; a `true` verdict proves the formula on every row |
| `witness_crt_sound` [Toledo: A3/M.14.v1] | the `crt` kernel: a witness `x` meeting every congruence `x ≡ rᵢ (mod mᵢ)` solves the simultaneous system |

## 22.4 The branch map — 230 solver kinds, exactly partitioned

> This is the **A2/A3 branch map** (230 kinds). The Hilbert-space core (Part XXI.5) adds a further
> **28** kinds — inner products, projections, Gram/Gram–Schmidt, operator norms, adjoint/self-adjoint
> tests, spectral decomposition — for 258; with the `polynomial_positivity` certificate solver the
> **live registry total is 259** (`idm.kinds()`). The partition
> below is the fold-generated branch half; it is not weakened by the Hilbert extension sitting beside it.
...
| Linear algebra / polynomials / stats | **A2** fold **with pivot** (§`pivot_preserves` [Toledo: A2/M.08.v1]) | 46 | `matrix_*`, `eigenvalues`, `linear_program`, `regression` |
...
`97 + 22 + 46 + 55 + 10 = 230` (branch partition verified against the pre-Hilbert registry; with the
28-kind Hilbert-space core of Part XXI.5 that is 258, and the `polynomial_positivity` certificate
solver brings the live registry to **259**).
...
**Local witnesses (this repo, `formal/*.v` — Coq 8.20, all axiom-free / *Closed under the global context*):** `formal/IDM_Keystone.v`: `keystone_B_eq_I` [Toledo: Keystone/M.03.v1] (Th 5.1, `B(Φ,Φ)=I(Φ)`) + `keystone_nonneg` [Toledo: Keystone/M.07.v1] (`L_R` PSD) + `relaxation_dissipation` [Toledo: Keystone/M.08.v1] (§21.3 no-blow-up). `formal/IDM_Logic.v`: `finite_satisfaction_dec` [Toledo: D/M.58.v1] (§10.3 Th 10.6, finite model-checking decidable) + `rdl_non_explosion` [Toledo: delta_R/M.04.v1] (§10.4/Part I, paraconsistent countermodel). `formal/IDM_FiniteWitnesses2.v`: `same_set_same_size` [Toledo: D/M.45.v1] (Th 10.3), `tape_count_succ` [Toledo: D/M.43.v1] (Th 10.4), `no_infinite_readout` [Toledo: D/M.42.v1] (Th 10.5), `lagrange_order_div` [Toledo: Z/M.10.v1] (§12.3). `formal/IDM_FiniteWitnesses.v`: `kuratowski_pair_inj` [Toledo: delta_R/M.02.v1] (Th 10.1, §10.1) · `handshake_lemma` [Toledo: L_R/M.01.v1] (§15.2) · `pigeonhole` [Toledo: D/M.40.v1] (§15.3) · `finite_yoneda` [Toledo: D/M.39.v1] (§17.2) · `semiring_distrib` [Toledo: D/M.41.v1] (§12.2). `formal/IDM_Geometry.v`: `orient_swap_bc/ab`, `orient_cyclic` [Toledo: L_R/M.05.v1], `orient_coincident_*`, `orient_translation` [Toledo: L_R/M.09.v1], `orient_scale` [Toledo: L_R/M.10.v1], `orient_collinear_mid/aff` (Part IV / §22.4, the exact orientation predicate the geometry solver branches on, 10 theorems). `formal/IDM_Reduction.v` (Part XXII, the root-design reductions, 42 theorems): `ftcc_Z` [Toledo: A2/M.03.v1], `foldmin_le_init/elem`, `foldmax_ge_init/elem`, `sum_is_fold` [Toledo: A2/M.06.v1], `path_is_fold` [Toledo: A2/M.07.v1], `fold_ext` [Toledo: A2/M.09.v1], `fold_linear` [Toledo: A2/M.10.v1], `fold_add_split` [Toledo: A2/M.11.v1], `dot_is_fold` [Toledo: A2/M.17.v1], `dot_scale` [Toledo: A2/M.18.v1], `pivot_preserves` [Toledo: A2/M.08.v1] (A2); `witness_sound/complete`, `decide_reflect` [Toledo: A3/M.06.v1], `decide_dec` [Toledo: A3/M.03.v1], `witness_composite_sound` [Toledo: A3/M.04.v1], `composite_has_factor` [Toledo: A3/M.05.v1], `witness_power_sound` [Toledo: A3/M.07.v1] (A3). `formal/IDM_ReadoutMinimality.v` + `IDM_ResolvedCount.v` (the signed-readout value-set + the resolved 0/⊥ inertia count), `formal/IDM_EquivariantReadout.v` (P1 necessary condition), `formal/IDM_Apriori.v` (the Richardson a-priori stability certificate), `formal/IDM_SetsFunctions.v` (Th 10.2, the function ≅ functional-relation coincidence), `formal/IDM_FirstOrder.v` (Th 10.6 first-order `⊨_λ`, decidable finite-domain satisfaction). One-command audit: `bash formal/verify.sh` (**174 theorems, all `Closed under the global context`**). Reproduce a single one: `cd formal && coqc -q IDM_Reduction.v` then `Print Assumptions ftcc_Z`.

**Sister-repo witnesses (`solver-arc-private/formal/*.v`, NOT local to this repo — verified
directly via `coqc -q RD.v` + `Print Assumptions <name>`):** `RDL.v` (RDL logic, 8 thm, `Th_coqc`) ·
`RD.v` + `RDL_Distinguishability.v` (D semiring/order/PA/`Con_PA`/`D≡ℕ`/`D⊨PA`/
discrete-floor, `Th_coqc`; `Con_PA_classical` `+classic`; see §2.3/§10.3 for the `D⊨PA`/`D≡ℕ` tier
note) · number ladder `ℤ/ℚ/ℝ`
