...
- **Part XXII** — The root design: three axioms that generate the solver (`READOUT · FOLD · DECISION` + the `KEYSTONE` bridge; the 230-kind branch map — 258 live with the Hilbert core; reductions machine-checked in `IDM_Reduction.v`)
...
solver `idm` (the 230-kind branch map; 258 kinds live once the Part XXI.5 Hilbert core is included), is produced. The claim of this part is deliberately narrow and tier-honest: **three axioms plus
...
> tests, spectral decomposition — for a **live registry total of 258** (`idm.kinds()`). The partition
...
28-kind Hilbert-space core of Part XXI.5 the live registry totals **258**).
