...
## 2.3 The naturals `D` (`Th_coqc`, axiom-free — witness `RD.v`, `RDL_Distinguishability.v` in the
sister repo `research_universal_solver/formal/`, NOT local to this repo's own `formal/`; verified
...
- **Th 2.7 (`Con_PA_classical`, `Th_coqc+classic`).** The classical frontier of consistency is closed
  ...
  `research_universal_solver/formal/RD_ConPA_ReadoutBivalence.v: Con_PA_classical_param` — verified
...
is `research_universal_solver/formal/RD.v` (sister repo, not local — `RD3_succ_ne_zero`,
...
- **Th 10.6 (`Th_coqc` — propositional witness `formal/IDM_Logic.v: finite_satisfaction_dec` / `sat_dec`, decidable model-checking over `¬∧∨`; FIRST-ORDER witness `formal/IDM_FirstOrder.v: eval_correct` / `sat_fo_decidable` — the quantifier-over-finite-domain Tarski recursion `∀↦forallb`, `∃↦existsb`, proved sound+complete and decidable, axiom-free):** `⊨_λ` — a decidable, computable Tarski recursion of satisfaction on a
  ...
  `research_universal_solver/formal/RD_ConPA_ReadoutBivalence.v` (`soundnessC_param`,
...
- **Downgrade (honest, tier note — see also §2.3 Th 2.5/Th 2.6):** the earlier "`D ⊨ PA`" (full
  ...
  **`+ℝ-Open`** here — they quantify over the completed standard model, and are NOT *reproven from
  ...
  (`sat_transfer`/`sentence_transfer`/`D_models_PA` in `research_universal_solver/formal/RD.v`) are
...
- **RD1–RD9 → `D ≅ ℕ`** (Peano-compatibility) · **RDL** 4-valued paraconsistent core (Belnap–Dunn FDE) ·
  ...
  **DEC toolkit** (discrete exterior calculus around `L_R`, 9 tools, minimal-cell scope) · **π and φ as
  ...
  `research_universal_solver/formal/`, not local to this repo: `RD.v`, `RDL.v`,
...
| `horner_scaled`, `horner_is_poly` | polynomial branch: Horner's method **is** a (right) fold and computes the polynomial `Σ cₖ xᵏ` exactly |
| `foldmin_in` | shortest-path exactness: the accumulated minimum is **attained** (equals the start or an actual element), not merely a lower bound |
| `fold_add_app` | the associative-monoid chunking law: a fold splits over concatenation ⇒ associative reductions parallelise (MapReduce), order-independent |
| `factorial_is_fold` | product / combinatorics branch: `n!` is the same fold over `(×)` |
...
| `witness_composite_sound`, `composite_has_factor` | number-theory kernel: a found divisor **proves** compositeness (the `is_prime` backbone) |
| `witness_power_sound` | `integer_root` / `is_perfect_square` kernel: a found `w` with `w^k=n` proves a perfect power |
| `witness_qr_sound`, `witness_dlog_sound` | crypto kernels: a found `w` with `w²≡a` / `g^w≡h (mod p)` solves the modular square root / discrete logarithm |
| `sat_reduces_to_decision`, `sat_model_sound` | the flagship decision problem: CNF **satisfiability** over the `2ⁿ` assignments is the A3 schema; a returned model provably satisfies every clause |
| `relax_nonincreasing`, `relax_idempotent` | Bellman–Ford / DP **convergence**: min-plus relaxation is non-increasing and idempotent per edge ⇒ the fixpoint is reached in finite rounds |
| `weak_duality_2` | **A2 × A3 bridge** — LP weak duality: a dual-feasible `y ≥ 0` is a *certificate* (A3) that bounds the simplex objective (A2), `c·x ≤ y·b` |
| `fold_right_perm` (+ `op_swap`) | the **deep semiring law**: a fold over an associative-commutative operation is invariant under any reordering (Permutation) of its inputs — the licence to reorder / chunk / parallelise, so A2 is well-defined independent of traversal order |
| `fold_split_even_odd` | the **FFT** divide-and-conquer split: the sum over `[0,2n)` decomposes into the even-index and odd-index sub-folds (the recursive decomposition the fast transform is built on) |
| `modpow_is_fold` | crypto forward map: modular exponentiation `g^N mod p` is the same fold over `(× mod p)` — the RSA / EC / discrete-log engine |
| `foldmax_in` | critical-/widest-path exactness: the accumulated maximum is **attained** (mirror of `foldmin_in`) |
| `sum_list_perm`, `prod_list_perm` | set-aggregation is **order-independent**: `Σ` and `Π` over a multiset are well-defined (corollaries of `fold_right_perm`) — the basis of `describe`/statistics and combinatorial products |
| `tautology_sound`, `cnf_tautology_sound` | the **universal** counterpart of the witness search — `truth_table` / validity: a bounded `∀` over the `2ⁿ` rows; a `true` verdict proves the formula on every row |
| `witness_crt_sound` | the `crt` kernel: a witness `x` meeting every congruence `x ≡ rᵢ (mod mᵢ)` solves the simultaneous system |
