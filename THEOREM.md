...
**Statement.** A first distinction exists: `succ 0 ≠ 0` (Th 2.1, `primordial_difference_exists`). The
engine `D` generated by Ax-RD1–RD9 is a commutative semiring `(D,⊕,⊗,0,1)` well-ordered by `≺`, and
carries a **discrete floor** `¬∃z, 0 ≺ z ≺ succ 0` (Th 2.9). Hence density is provably absent at the
root: a continuum can enter only as a later readout, never as a primitive (Pr 2.1).
...
semiring — not a postulate about the continuum.
