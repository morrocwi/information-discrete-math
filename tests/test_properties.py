...
FIXTURES = {
    ...
    "rational_limit": {"num": [-1, 0, 1], "den": [-1, 1], "point": 1},
    "linear_ode": {"coeffs": [2, -3, 1]},
    "groebner_basis": {"polys": ["x^2+y^2-1", "x-y"], "variables": ["x", "y"], "order": "grevlex"},
    ...
    "fixed_point": {"g": "cos(x)", "x0": 1}, "summation": {"term": "n**2", "a": 1, "b": 10},
    "regularized_sum": {"power": 1},
    # limits / series
    "limit": {"seq": "(1+1/n)**n"}, "limit_infinity": {"f": "(2*x+1)/(x+1)"},
    ...
    "taylor_series": {"f": "exp(x)", "x0": 0, "n": 5}, "laurent_series": {"f": "1/x", "x0": 0, "lo": -2, "hi": 2},
    "pade": {"coeffs": ["1", "1", "1/2", "1/6", "1/24"], "m": 2, "n": 2}, "geometric_series": {"r": "1/3", "eps": "1e-12"},
    "fourier_series": {"f": "x", "n": 3, "period": 6.283185307},
    # transforms / complex
    "laplace_transform": {"f": "exp(2*t)", "s": 3}, "inverse_laplace": {"F": "1/(s-1)", "t": 1},
    "mellin_transform": {"f": "exp(-t)", "s": 2}, "fourier_transform": {"f": "exp(-t**2)", "omega": 1, "T": 10},
    ...
    "residue_integral": {"num": [1], "den": [1, 0, 1]},
    # symbolic
    ...
    "pde_wave": {"init": "sin(pi*x)", "init_vel": "0", "c": 1, "L": 1, "T": "0.1", "Nx": 10,
                 "bc": [0, 0], "at": 0.5},
    "pde_laplace": {"box": [0, 1, 0, 1], "Nx": 8, "Ny": 8, "bc": 1, "at": [0.5, 0.5]},
    "pde_poisson": {"f": "0", "box": [0, 1, 0, 1], "Nx": 8, "Ny": 8, "bc": 1, "at": [0.5, 0.5]},
    ...
    "ns_retained_rk4": {"K": 1, "nu": 0.005, "dt": 0.0025, "horizon": 1, "target_mode": [1, 0, 0], "verify": True},
