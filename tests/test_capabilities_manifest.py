...
def test_domain_union_covers_every_kind_exactly_once():
    ...
    # The manifest is single-sourced against the LIVE registry — no hardcoded total to drift. The two
    # asserts above (manifest == idm.kinds(), no duplicates) already pin the count to len(idm.kinds()).
    assert len(seen) == len(live_kinds)
